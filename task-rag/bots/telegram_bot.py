...
from aiogram.types import BotCommand, Document, Message, Update, WebhookInfo
...
class TelegramBot:
    ...
    def _setup_handlers(self):
            ...
            try:
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.get(
                        f"{self.fastapi_base_url}/healthz"
                    ) as response:
        ...
        async def text_handler(message: Message):
            ...
            try:
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.post(
                        f"{self.fastapi_base_url}/chat/", json={"message": user_text}
                    ) as response:
                        if response.status == 200:
    ...
    async def _handle_streaming_response(
        self, session_id: str, message: Message, original_text: str
    ):
                                ...
                                if msg_type == "token":
                                    ...
                                    if (
                                        ...
                                        try:
                                            ...
                                            logger.warning(
                                                f"Message edit error: {edit_error}"
                                            )
    ...
    async def set_webhook(self) -> bool:
        ...
        try:
            ...
            await self.bot.set_webhook(
                url=webhook_url, secret_token=self.settings.telegram_webhook_secret
            )
