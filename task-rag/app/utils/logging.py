...
def log_function_call(
    func_name: str, args: Optional[dict] = None, kwargs: Optional[dict] = None
):
...
class TimedOperation:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if exc_type is None:
            self.logger.info(
                f"Operation completed: {self.operation_name} in {duration:.3f}s"
            )
        else:
            self.logger.error(
                f"Operation failed: {self.operation_name} after {duration:.3f}s"
            )
