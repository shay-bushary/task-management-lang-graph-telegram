...
class TelegramService:
    ...
    async def edit_message(
        ...
        try:
            ...
            return await self.edit_message(
                chat_id, message_id, text, parse_mode, disable_web_page_preview
            )
    ...
    async def handle_streaming_chat(
        self, chat_id: int, user_message: str, initial_message_id: Optional[int] = None
    ) -> Optional[Message]:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    f"{self.fastapi_base_url}/chat/", json={"message": user_message}
                ) as response:
            ...
            return await self._handle_websocket_streaming(
                chat_id=chat_id, message_id=initial_message_id, session_id=session_id
            )
    ...
    async def _handle_websocket_streaming(
        self, chat_id: int, message_id: int, session_id: str
    ) -> Optional[Message]:
                    ...
                    async for msg in ws:
                        if msg.type == aiohttp.WSMsgType.TEXT:
                            try:
                                ...
                                if msg_type == "token":
                                    ...
                                    if (
                                        ...
                                        if len(display_text) > max_message_length:
                                            display_text = (
                                                display_text[: max_message_length - 50]
                                                + "..."
                                            )
                                    ...
                                    if len(final_content) > max_message_length:
                                        final_content = (
                                            final_content[: max_message_length - 50]
                                            + "..."
                                        )
                                ...
                                logger.warning(
                                    f"Invalid JSON from WebSocket: {msg.data}"
                                )
                                continue
    ...
    async def handle_document_upload(
        self, chat_id: int, file_id: str, filename: str
    ) -> Optional[Message]:
        ...
        try:
            ...
            form_data.add_field(
                "file", file_content, filename=filename, content_type="application/pdf"
            )

            # Send to FastAPI ingestion endpoint
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    f"{self.fastapi_base_url}/ingest/pdf", data=form_data
                ) as response:
                    if response.status == 200:
