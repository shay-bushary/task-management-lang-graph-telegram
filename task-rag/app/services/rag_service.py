...
class RAGService:
    ...
    def process_pdf(self, file_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Successfully processed PDF {file_path.name}: {len(chunks)} chunks added"
            )
    ...
    def search_documents_with_scores(
        ...
        try:
            ...
            logger.info(
                f"Searching documents with scores for query: '{query}' (k={search_k})"
            )
    ...
    def delete_documents_by_source(self, source: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Deleted {len(results['ids'])} documents from source: {source}"
            )
