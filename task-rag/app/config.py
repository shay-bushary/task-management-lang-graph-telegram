...
class Settings(BaseSettings):
    ...
    model_name: str = Field(
        ...
        description="OpenAI model name for chat completion",
    )
    embeddings_model: str = Field(
        default="text-embedding-3-small", description="OpenAI embeddings model"
    )
    ...
    chroma_dir: Path = Field(
        default=Path("data/chroma"), description="Directory for Chroma vector database"
    )
    ...
    chunk_size: int = Field(
        default=1000, description="Text chunk size for document splitting"
    )
